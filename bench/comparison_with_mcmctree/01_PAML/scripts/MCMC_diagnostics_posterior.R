...
delcol_post <- 2 # There are 2 columns as we have 1 alignment block: 
                 # mu, sigma2
...
# of chains we ran).
num_clocks <- 1
names( num_clocks ) <- "ILN"
all_paths <- vector( mode = "character",
                     length = c( length( dat ) )*num_clocks )
count <- 0
  ...
  if( i > 1 ){
    ...
    if( num_clocks == 2 ){
      ...
      all_paths[count] <- paste( home_dir, "01_MCMCtree/01_posterior/ILN/",
                                 i, "/", sep = "" )
    }else if( num_clocks == 1 && names( num_clocks ) == "ILN" ){
      all_paths[count] <- paste( home_dir, "01_MCMCtree/01_posterior/ILN/",
                                 i, "/", sep = "" )
    }
    else if( num_clocks == 1 && names( num_clocks ) == "GBM" ){
      all_paths[count] <- paste( home_dir, "01_MCMCtree/01_posterior/GBM/",
                                 i, "/", sep = "" )
    }
  }else{
...
if( num_clocks == 2 ){
...
}else if( num_clocks == 1 ){
  dataset <- sort( c( paste( dat, "_", names( num_clocks ), sep = "" ) ) )
  node_calib         <- vector( mode = "list", length = length( dataset ) )
  names( node_calib) <- dataset
  # Get only the matrices, not the list, hence the `[[1]]`
  # If you had more datasets, you could continue with `node_calib[[3]]`, etc.
  ...
  out_file_pdf  <- sort( c( paste( "Convergence_plot_post_", dat, "_",
                                   names( num_clocks ), sep = "" ) ) )
  out_title_pdf <- rep( c( "GBM","ILN" ), length(dat) )
}
perc          <- perc
