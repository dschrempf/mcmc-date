...
data RelaxedMolecularClockModel
  = UncorrelatedGamma
...
priorFunctionRelaxedMolecularClock ht md t' x =
  product'
    [ -- Mean rate. The mean of the mean rate ^^ is (1/height).
      ...
      --
      -- Use a convex gamma distribution to avoid pathological situations with
      -- the exponential distribution where the variance of the rate becomes
      -- zero (or close to zero) and all rates become nearly the same.
      gamma (3 / 2) (1 / 6) va,
      -- Relative rate tree.
      case md of
        UncorrelatedGamma -> uncorrelatedGamma WithoutStem 1.0 va r
        UncorrelatedLogNormal -> uncorrelatedLogNormal WithoutStem 1.0 va r
        UncorrelatedWhiteNoise -> uncorrelatedWhiteNoise WithoutStem 1.0 va t' r
