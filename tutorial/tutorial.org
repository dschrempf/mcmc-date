...
branch is older than the donor branch.
...
have similar ages. In this tutorial we will not brace nodes but see Appendix
[[id:73cefaee-838c-451e-9dbc-a80b4b7123f4]] for the specifications.
...
particular, we will use Phylobayes [cite:@Lartillot2013]. In this step, we have
to decide on an evolutionary model. The optimal evolutionary model is determined
by the nature (for example, the time range between most recent common ancestor
and the leaves of the tree) and the size of the data set (that is, number of
rows and columns in the alignment) which is an indicator for the computational
requirements.
...
but more complex to fast but simpler: GTR+CAT+G4, LG+CAT+G4 [cite:LG: @Le2008a],
LG+EDM64+G4 [cite: EDM: @Schrempf2020a], LG+C60+G4 [cite: C60: @Quang2008], and
LG+G4.
...
Usage: mcmc-date-run run --analysis-name NAME [--preparation-name NAME]
                         ...
                         --relaxed-molecular-clock ARG [--seed NUMBER] [--mc3]
...
Available options:
  ...
  --seed NUMBER            Set the seed (default: unset, random).
