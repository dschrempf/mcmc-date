...
- with a comma separated values (CSV) file, or
- with a Newick tree file (MCMCTree specification; see the documentation of
  MCMCTree; only L, U, and B are supported).
If the file ends with =csv=, assume the calibrations are provided in CSV format.
Otherwise, assume the calibrations are provided on a Newick tree . The CSV file
has rows of the following format:
#+begin_src text :exports code
CalibName,LeafA,LeafB,LowerBoundary,LowerBoundaryProbMass,UpperBoundary,UpperBoundaryProbMass
#+end_src
...
lower and upper boundary. The probability mass describes the softness (or
hardness) of a boundary. In other words, the probability mass describes the
steepness of the decline of the prior function outside the calibration interval.
In general, the larger the probability mass the softer the boundary. We specify
the probability mass with respect to a normalized time interval of size \(1.0\).
That is, probability masses have to be strictly positive and less than \(1.0\),
which is the total probability mass in the unit interval.
...
corresponding probability mass. For example, the following line defines a node
