...
with node labels and branch lengths in which not more than one branch
...
elapsed absolute time units. For a given branch,
\begin{align}
\begin{split}
...
between parameter tuples \((t, r)\) and \((t', r')\), where
\begin{align}
  t' &= t \cdot c, \\
  r' &= r / c,
\end{align}
...
than within \(0.45\;\text{My}\). Of note, if the root is younger, e.g., at
\(2.5\;\text{Gya}\), then a value of \num{1e-4} is stricter in terms of absolute
time units, and roughly corresponds to an interval of \(0.25\;\text{My}\).
