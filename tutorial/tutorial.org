...
lengths for this topology which are measured in absolute time (for example, in
...
Sometimes, all we can do is date a tree with relative time units (Section
...
measured in real time. The situation is severe because \(r\) and \(t\) are
confounded! If we multiply \(t\) with a positive number \(c\) and divide \(r\)
by the same positive number \(c\), the distance \(d\) stays constant. Since the
likelihood only depends on the distance \(d\) we can not discriminate between
parameters
\begin{align}
  t' &= t \cdot c, \\
  r' &= r / c,
...
:ID:       b1d8692b-db95-42f9-a11d-e492e6debf26
:END:
This means we need auxiliary data to constrains the ages of at least some
...
Without node age calibrations we can still date the tree using branch lengths
measured in /relative time/. In particular, we achieve this by fixing the tree
height to be 1.0.
