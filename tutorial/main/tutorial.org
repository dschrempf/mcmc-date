...
#+title: Dating phylogenetic trees with McmcDate
#+subtitle: A detailed tutorial
...
Section [[id:e3995389-bebf-4e0f-be48-6ca1a9e87c6c]] describes the example analysis.
If you want to follow the tutorial on your computer, install the Haskell
toolchain and clone the McmcDate repository as described in Appendix
...
* Example analysis
:PROPERTIES:
:ID:       e3995389-bebf-4e0f-be48-6ca1a9e87c6c
:END:
...
(Appendix\nbsp{}[[id:a38b78be-7ee6-4340-946a-2a5d06385b66]]) and you need to install
the Haskell toolchain. Luckily, installation is easy with GHCup. Follow the
[[https://www.haskell.org/get-started/][instructions on the Haskell website]]. I recommend using =cabal-install=, see also
Appendix\nbsp{}[[id:a38b78be-7ee6-4340-946a-2a5d06385b66]].

Here, we will use a [[https://github.com/dschrempf/mcmc-date/blob/master/scripts/run][wrapper script]] around \mcmcdate{}. The wrapper script has a
simple command-line user interface and also takes care of compiling the
executable provided the Haskell toolchain has been installed properly. The
wrapper script uses a configuration file which is a BASH script assigning some
variables:
...
internal settings (Appendix\nbsp{}[[id:0d0620bf-1fc4-484c-b7b4-d6ffffe0b357]]). If
you want to sample more points from the posterior function, you can continue the
analysis:
#+name: step4-mcmcdate-continue
#+begin_src sh :exports code :eval never
./step4-mcmcdate -f step4-strassert.conf -c -k ul s c
#+end_src

\noindent If you repeat an analyses with similar parameters (but the same set of
MCMC proposals), you can use the option =--init-from-save=
(Appendix\nbsp{}[[id:d753cace-4f9c-4212-a7c7-7e09cb8c9a80]]).
...
sometimes sub-optimal. Before running \mcmcdate{} you need to (1) install the
Haskell toolchain and (2) compile the Haskell code.

1. Installing the Haskell toolchain: I recommend using [[https://www.haskell.org/ghcup/][GHCup]], a cross platform
   script handling the installation process. Check the website for instructions.
   Note, there are two build tools commonly used with Haskell: =cabal-install=,
   and Haskell =Stack= with binaries =cabal=, and =stack=, respectively. I
   recommend using =cabal-install= (see below).

2. Compiling the source code: Clone the source code of McmcDate:
   #+name: CloneMcmcDate
   #+begin_src sh :exports code
   git clone git@github.com:dschrempf/mcmc-date.git
   #+end_src
   Perform your analysis within the folder of the McmcDate repository. For
   example, run test analyses or the tutorial analysis from within the
   =mcmc-date/tests= or =mcmc-date/tutorial/data= directory, respectively. In
   this case, the [[https://github.com/dschrempf/mcmc-date/blob/master/scripts/run][wrapper script]], which is used in this tutorial, compiles and
   runs the source code for you in a reproducible way, and so there is no need
   for manual action. Sometimes, however, manual action may be required. In this
   case, you need a rough understanding of the tools involved. For example, the
   wrapper script uses =cabal-install= by default. If you want to use =Stack=,
   use the option =-s= like so: =run -s ...=. See also the output of =run -h=.
   In rare occasions you may want to clean your local build cache. You can do
   this by running =cabal clean= or =stack clean=; or more strictly, by deleting
   the =dist-newstyle= or =.stack-work= directories in you working directories
   for =cabal-install= and =Stack=, respectively.
